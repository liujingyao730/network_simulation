...
class dyn_embedding(nn.Module):

    def __init__(self, args):

        super(dyn_embedding, self).__init__()
        ...
        self.dyn_embedding_layer = attention_on_node({
            "output_size": self.dest_size,
            "encoding_size": self.dest_size,
            "gnn": self.gnn_type
        })
        ...
        self.state_gate = nn.Linear(self.input_size-5*self.dest_size, 6*self.dest_size)
        self.sigmoid = nn.Sigmoid()

        self.rnn_type = args.get("rnn", "gru")
        if self.rnn_type == "gru":
            self.temporal_cell = nn.GRUCell(6*self.dest_size, self.hidden_size)
        elif self.rnn_type == "lstm":
            self.temporal_cell = nn.LSTMCell(6*self.dest_size, self.hidden_size)
        elif self.rnn_type == "conv":
            raise NotImplementedError
        else:
            raise NotImplementedError
    ...
    def infer(self, input_data, adj_list, mod="infer"):
        ...
        for i in range(temporal-1):
            ...
            dyn_embedding = self.dyn_embedding_layer(
                dyn_feature, dyn_feature,
                laplace_list_forward[i, :, :], laplace_list_backward[i, :, :]
            )
            tmp_input = torch.cat((lane_number_embedding, dir_embedding, dyn_embedding), dim=2)
            ...
            if self.rnn_type == "gru":
                hidden = self.temporal_cell(
                    torch.reshape(tmp_input, (batch * cell, 6 * self.dest_size)),
                    torch.reshape(h_space, (batch * cell, self.hidden_size))
                )
            elif self.rnn_type == "lstm":
                hidden, c = self.temporal_cell(
