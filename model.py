...
class GCN_GRU_2(nn.Module):
    def __init__(self, args, input_cells=None):

        super(GCN_GRU_2, self).__init__()
        self.input_size = args["input_size"]
        self.output_size = args["output_size"]
        self.hidden_size = args.get("hidden_size", 64)
        self.init_length = args.get("init_length", 4)
        ...
        self.gnn_type = args.get("gnn", "gcn")
        if self.gnn_type == "gcn":
            self.init_graph = gcn(self.input_size, 2 * self.hidden_size)
            self.forward_gnn = gcn(2 * self.hidden_size, self.hidden_size)
            self.backward_gnn = gcn(2 * self.hidden_size, self.hidden_size)
        else:
            raise NotImplementedError

        self.rnn_type = args.get("rnn", "gru")
        if self.rnn_type == "gru":
            self.temporal_cell = nn.GRUCell(self.input_size, 2 * self.hidden_size)

        self.output_layer = nn.Linear(2*self.hidden_size, self.output_size)
    ...
    def forward(self, input_data, adj_list):
        ...
        for i in range(temporal):
            ...
            hidden = self.temporal_cell(
                torch.reshape(input_data[:, i, :, :], (batch * cell, feature)),
                torch.reshape(h_space, (batch * cell, 2*self.hidden_size)))
            hidden = hidden.view(batch, cell, 2 * self.hidden_size)
            if i >= self.init_length:
